...
from app.core.models import Environment as EnvironmentModel
from app.core.models import UserEnvironment
...
def _join_request_out(req, session: Session) -> JoinRequestOut:
    ...
    return JoinRequestOut(
        ...
        requester_avatar_key=u.avatar_key if u else None,
...
@router.get("/for-user/{user_id}", response_model=list[EnvironmentModel])
...
def list_environments(session: Session = Depends(get_session)) -> list[EnvironmentModel]:
...
def get_environment_members(
    ...
    return [EnvironmentMemberOut.model_validate(r) for r in raw]
...
def get_join_requests(
    ...
    rows = smart_home_service.list_join_requests(
        environment_id, admin_user_id, session
    )
    return [_join_request_out(r, session) for r in rows]
...
def post_join_request(
    ...
    req = smart_home_service.create_join_request(
        environment_id, payload.user_id, session
    )
...
def add_user_to_environment(
    ...
    return smart_home_service.add_user_to_environment(
        environment_id, payload.user_id, session
    )
